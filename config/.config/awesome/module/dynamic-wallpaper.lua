...
local wall_config = {
	...
	wall_dir = filesystem.get_xdg_config_home() .. (config.module.dynamic_wallpaper.wall_dir or 'wallpapers/'),
...
if #wall_config.wallpaper_schedule == 0 then
	...
	if count == 0 then --Schedule is actually empty
		-- Get all pictures
		local pictures = dir.filter_files_by_format(dir.get_dir_contents(wall_config.wall_dir), wall_config.valid_picture_formats)
...
-- Change colorscheme using pywal if pywal available
local set_colorscheme_pywal = function(path)
    awful.spawn.with_shell('which wal && wal -i "' .. path .. '"')
end
...
local update_wallpaper = function(wall_name)
    ...
    set_colorscheme_pywal(wall_dir)
